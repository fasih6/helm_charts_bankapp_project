...
```bash
helm install bankapp ./bankapp-chart -f custom-values.yaml
```

**Installation in a specific namespace:**
```bash
kubectl create namespace bankapp
helm install bankapp ./bankapp-chart --namespace bankapp
```
...
helm upgrade bankapp ./bankapp-chart -f custom-values.yaml
...
helm uninstall bankapp --namespace bankapp
```
...
```bash
export SERVICE_IP=$(kubectl get svc bankapp-service -o jsonpath='{.status.loadBalancer.ingress[0].hostname}')
